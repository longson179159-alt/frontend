<template>
  <div class="w-[420px] max-w-[420px] rounded-3xl border border-gray-200 bg-white shadow-2xl">
    <div class="flex flex-col rounded-3xl pt-3">
      ...
      <div class="px-5 pb-2">
        <div class="my-3 flex flex-row items-center gap-2 text-xl font-bold">
          <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full hover:bg-gray-300"
        ...
        <div class="flex">
          ...
          <div class="flex flex-wrap items-center gap-1 border-l border-gray-400 px-2 text-xs">
            ...
            <button v-if="!openAddTag" type="button" class="rounded border border-gray-400 px-1 hover:bg-gray-300"
              @click="openAddTag = true">
              Tag +
            </button>

            <input v-else v-model="newTag" type="text"
              class="w-24 rounded border border-gray-300 px-2 py-0.5 focus:outline-none" placeholder="New tag"
      ...
      <div class="flex  flex-1 flex-col gap-1 overflow-auto border-y border-y-gray-300 p-5">
        ...
        <div class="max-h-44 overflow-auto">
          <div v-for="(meaning, index) in yourMeanings" :key="`saved-${index}`" class="group relative mt-2">
            <textarea v-model="yourMeanings[index]" rows="2"
              class="w-full rounded-lg bg-gray-100 px-2 pt-2 text-start leading-none focus:outline-none focus:ring-0"
              placeholder="Enter meaning" @input="autoResize"
            ...
            <div class="absolute right-3 top-1/2 hidden -translate-y-1/2 gap-1 group-hover:flex">
              <button type="button" class="flex h-5 w-5 items-center justify-center rounded-full bg-white"
        ...
        <textarea v-show="openAddMeaning || yourMeanings.length === 0" v-model="newMeaning" rows="2"
          class="mt-3 min-h-10 w-full rounded-lg border px-2 pt-2 text-start leading-none focus:outline-none focus:ring-0"
          placeholder="Enter new meaning, then press Enter" @input="autoResize"
          @keydown.enter.prevent="addMeaning"></textarea>
      ...
      <div class="flex justify-between px-4 py-2">
        <button v-for="status in statusButtons" :key="status.value" type="button"
          class="flex h-10 w-10 items-center justify-center rounded-full border border-gray-300" :class="[
      ...
      <div class="flex  flex-col pb-3 pt-1">
        <div class="flex justify-end gap-3 px-5 pb-2 pt-1">
          <button @click="emit('closePopup', false)" type="button"
            class="rounded-lg w-20 border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100">
            Cancel
          </button>
          <button @click="saveWord" type="button"
            class="rounded-lg w-20 bg-[#0B1B32] px-4 py-2 text-sm font-medium text-white hover:bg-black">
            Save
          </button>
        </div>

        <div v-if="messageWarning" class="inline-flex max-w-52 truncate text-red-500 self-end mr-5  text-sm italic">{{ messageWarning }}</div>
      </div>
    </div>
  </div>
</template>
...
const emit = defineEmits(['closePopup', 'updateParentData'])
...
const saveWord = async () => {
  ...
  try {
    ...
    emit('updateParentData', playLoad)
    emit('closePopup', false)
  } catch (error) {
    // console.error('Failed to update word data:', error)
    messageWarning.value = error?.data?.message ?? 'Error with updating word data'
