      ...
      <div class="relative flex w-80 ">
        ...
        <div ref="coursePopup" v-if="openToggleCourse" class="absolute flex flex-col top-full left-0 w-full max-h-60   z-10">
          ...
          <div class="border rounded-lg bg-white shadow-md overflow-auto">
            <button
              v-for="course in listCourseNames"
              v-show="includesearchCourse(course)"
              @click="currentCourseName = course; openToggleCourse = false; searchCourse = ''; currentLessonName = 'All'"
              :key="course"
        ...
        <div ref="lessonPopup" v-if="openToggleLesson" class="absolute flex flex-col top-full left-0 w-full max-h-60   z-10">
          ...
          <div class="border rounded-lg bg-white shadow-md overflow-auto">
            <button
              v-for="lesson in listLessonNames"
              v-show="includeSearchLesson(lesson)"
              @click="currentLessonName = lesson; openToggleLesson = false; searchLesson = ''"
              :key="lesson"
...
watch([currentCourseName, currentLessonName], ([newCourse, newLesson]) => {
  ...
  emit('selectionChanged', { courseName: newCourse, lessonName: newLesson })
})

onMounted(() => {
   getDataBackend()
   emit('selectionChanged', { courseName: currentCourseName.value, lessonName: currentLessonName.value })
